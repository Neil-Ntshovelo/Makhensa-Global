...
import { isRunningNow, formatShortDate } from "../utils/date";
...
const CurrentOpportunities = () => {
  const running = useMemo(() => {
    return opportunities
      .filter((o) => isRunningNow({ startDate: o.startDate, endDate: o.endDate }))
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-16">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10">
        ...
        <Link
          to="/programs"
          className="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-yellow-400 text-blue-900 font-semibold clay-pressable"
        >
          View All
        </Link>
      </div>
      ...
      {running.length === 0 ? (
        <ClayCard className="p-8">
          <p className="text-gray-700">No current learnerships/internships found.</p>
        </ClayCard>
      ) : (
        <div className="grid md:grid-cols-3 gap-6">
          {running.map((o) => (
            <ClayCard key={o.id} className="p-7 transition-transform hover:-translate-y-1">
              ...
              <p className="text-gray-600 mt-4">{o.description}</p>
